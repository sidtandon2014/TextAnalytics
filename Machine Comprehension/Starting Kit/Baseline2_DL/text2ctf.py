...
def splitDataset(train,trainFilename):
    import pandas as pd
    dataFilePath = "F:/Sid/Learnings/Data Scientist/Text Analysis/Machine Comprehension/data/data.tsv"
    data = pd.read_csv(dataFilePath, header = None, sep = "\t")
    train, test = train_test_split(data, test_size=0.10, random_state=42)
    folderPath = "F:/Sid/Learnings/Data Scientist/Text Analysis/Machine Comprehension/Starting Kit/Baseline2_DL/traindata.tsv"
    train.to_csv(folderPath,index = False,header = None,sep = "\t")
    folderPath = "F:/Sid/Learnings/Data Scientist/Text Analysis/Machine Comprehension/Starting Kit/Baseline2_DL/validationdata.tsv"
    test.to_csv(folderPath,index = False,header = None,sep = "\t")
    

if __name__ == "__main__":
    
    folderPath = "F:/Sid/Learnings/Data Scientist/Text Analysis/Machine Comprehension/Starting Kit/Baseline2_DL/"
    trainFileName = folderPath + "traindata.tsv"
    validationFileName = folderPath + "validationdata.tsv"
    EvaluationFileName = folderPath + "eval1_unlabelled.tsv"

    embeddingFileName = folderPath + "glove.6B.50d.txt"
    ...
    TextDataToCTF(trainFileName,folderPath + "TrainData.ctf",False)
    print("Train Data conversion is done")
    TextDataToCTF(validationFileName,folderPath + "ValidationData.ctf",False)
    print("Validation Data conversion is done")
    TextDataToCTF(EvaluationFileName,folderPath + "EvaluationData.ctf",True)
