...
def getRootWord(data):
    from nltk.stem.porter import PorterStemmer
    porter_stemmer = PorterStemmer()
    
    return data.Words.apply(lambda x: porter_stemmer.stem(x.lower()))

def replaceWordWithRootWord(data):
    from nltk.stem.porter import PorterStemmer  
    porter_stemmer = PorterStemmer()
    ...
    for index in data.index:
        statement = []
        phrase = data.iloc[index,].Phrase
        for word in phrase.split():
            word_lower = word.lower()
            statement.append(porter_stemmer.stem(word_lower))
        sentences.append(' '.join(statement))
    return pd.Series(sentences)
        
def removeStopWordsAndReplaceWithRootWordFromString(statement,myStopWords):
    from nltk.stem.porter import PorterStemmer
    ...
    for word in statement.split():
        word_lower = word.lower()
        if word_lower not in myStopWords:
            sentence.append(porter_stemmer.stem(word_lower))
...
def cleanFeeds(data, minFrequency,myStopWords,isTraining = True):
    ...
    for index in data.index:
        ...
        tmpStmt = removeStopWordsAndReplaceWithRootWordFromString(review,myStopWords)
        if len(tmpStmt) > 0:
...
def generateTokens(data,data_Full,n_dim,myStopWords):
    ...
    countVectorizer = CountVectorizer(ngram_range = (1,2),stop_words = myStopWords,max_df = .8)
    nmf = TruncatedSVD(n_components = n_dim)
...
def mergeFeatureSet(W2Vmodel,tfIdModel):
    return np.concatenate((W2Vmodel,tfIdModel),axis = 1)
...
#---------Take root word
positiveWords = pd.DataFrame({"Words":getRootWord(positiveWords).unique()})
negativeWords = pd.DataFrame({"Words":getRootWord(negativeWords).unique()})
neutralWords = pd.DataFrame({"Words":getRootWord(neutralWords).unique()})
...
train_Dataset = train_Dataset[train_Dataset.Sentiments!=2]
...
featureSet = mergeFeatureSet(featureSet,tfFeatureSet)
"""
Split the data into training and testing set
"""

#--------Use tfID Vectorizer feature set
train_x,test_x,train_y,test_y = train_test_split(featureSet,train_Dataset.Sentiments,test_size = 0.1)
...
print(classification_report(test_y,y_predict))
print(modelDT.score(test_x,test_y))
...
y_predict = logitmodel.predict(test_x)
print(logitmodel.score(test_x,test_y))
print(classification_report(test_y,y_predict))

"""
SVC taking lot of time while training
57% with tfidVEctorizer and lnm
62% with 2 classes
...
y_predict = svcModel.predict(test_x)
print(svcModel.score(test_x,test_y))
print(classification_report(test_y,y_predict))  
...
maxent = MaxentClassifier.train(featureSet)
maxent.train(featureSet)
...
y_predict = multinomialModel.predict(test_x)
print(multinomialModel.score(test_x,test_y))
print(classification_report(test_y,y_predict))
...
estimators = []
estimators.append(('MultinomalNB', MultinomialNB()))
estimators.append(('cart', DecisionTreeClassifier()))
estimators.append(('svm', SVC()))
estimators.append(("Random Forest",RandomForestClassifier(max_features=100)))
# create the ensemble model
ensemble = VotingClassifier(estimators)
ensemble.fit(train_x, train_y)
y_predict = ensemble.predict(test_x)
print(classification_report(test_y,y_predict))
